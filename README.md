...
**Status:** phases 1–3 are largely built. The gateway boots, serves the HUD, enforces every risk
rule, places and closes orders, and journals plans, events, and closed trades with an auditable R;
the HUD connects, streams quotes, and drives the pad FSM, overlay, and telemetry. Neither has
spoken to Spotware, and the pad has not been tested on real hardware. Run the whole thing today
with `broker.transport: mock`, which answers real Protobuf in process — see
[`docs/gateway.md`](./docs/gateway.md) and [`docs/hud.md`](./docs/hud.md). The authority for
everything below is
...
| 2 | [cTrader exec and socket gateway](./plans/260824-1506-evening-forex-gold-gamepad/phase-02-ctrader-exec-and-socket-gateway.md) | 22h | **Code complete** — needs the acceptance run against a real demo account |
| 3 | [Web game and 8BitDo client agent](./plans/260824-1506-evening-forex-gold-gamepad/phase-03-web-game-and-8bitdo-client-agent.md) | 18h | **Partial** — manual pad pass needs hardware |
...
| 7 | [Playbook, rule registry, trade grading](./plans/260824-1506-evening-forex-gold-gamepad/phase-07-playbook-and-trade-grading.md) | 12h | **Done** |
...
| 9 | [Tilt telemetry and adaptive friction](./plans/260824-1506-evening-forex-gold-gamepad/phase-09-tilt-telemetry-and-adaptive-friction.md) | 10h | **Done** — voice arousal waits on phase 8 |
| 10 | [Trade replay](./plans/260824-1506-evening-forex-gold-gamepad/phase-10-trade-replay.md) | 12h | **Partial** — serving done; replay surface pending |
| 11 | [Process Score and radar deck](./plans/260824-1506-evening-forex-gold-gamepad/phase-11-process-score-and-radar-deck.md) | 8h | **Partial** — scoring done; radar deck is phase 6 |
...
`127.0.0.1:8444`. `pnpm -C app dev` runs the HUD on 5173 with `/ws` and `/api`
proxied to the gateway.

**Tab must stay focused.** Chrome stops delivering pad input to an unfocused
window on some systems; the HUD shows the lock rather than pretending otherwise.
Link the pad with the 2.4G dongle (wired USB is the fallback) — Bluetooth on the
Ultimate 2 needs macOS 26+.
