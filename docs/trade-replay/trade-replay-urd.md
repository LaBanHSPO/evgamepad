...
|---|---|---|---|---|
| A-01 | ~~Việc **tự đóng băng bối cảnh quanh mỗi lệnh** (UN-014) thuộc feature này~~ — **Resolved 2026-08-29: thuộc `order-execution`** (FR-059..063). Feature này **chỉ đọc** tape; UN-014 trở thành một phụ thuộc, không còn là phạm vi. Lý do: `phase-02` vốn đặt vòng đệm ở đó · journal path bị cấm đi trên order socket · và tape tích luỹ từ phiên đầu thay vì từ ngày feature này ship | Đổi hoàn toàn quy mô công việc: "màn đọc dữ liệu có sẵn" hay "màn đọc **cộng** hạ tầng ghi chạy nền quanh mọi lệnh". Sai chỗ thì cả ước lượng công sức lẫn ranh giới Mục 3 đều lệch | Chưa xác nhận — `order-execution-urd.md` Mục 3 không nhận việc này, mà replay là nơi duy nhất tiêu thụ nó | **Chốt trước khi viết SRS, không để trôi.** Nếu chuyển sang `order-execution` thì UN-014 rời khỏi doc này và thêm một dòng Out of Scope |
